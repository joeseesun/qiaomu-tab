(function () {
  ...
  async function fetchFavicon(url) {
    ...
    const directIcon = await fetchDeclaredFavicon(normalizedUrl);
    if (directIcon) {
      return directIcon;
    }
    const rootIcon = await fetchIconCandidate(new URL("/favicon.ico", normalizedUrl).href);
    if (rootIcon) {
      return rootIcon;
    }
  ...
  async function fetchDeclaredFavicon(url) {
    try {
      ...
      const candidates = getIconCandidatesFromDocument(doc, response.url || url);
      for (const candidate of candidates) {
        const dataUrl = await fetchIconCandidate(candidate);
        if (dataUrl) {
          return dataUrl;
        }
      }
  ...
  function getIconCandidatesFromDocument(doc, baseUrl) {
    const links = Array.from(doc.querySelectorAll("link[rel][href]"));
    const directIcons = links
      .filter((link) => {
        const rel = (link.getAttribute("rel") || "").toLowerCase();
        return rel.includes("apple-touch-icon") || rel.split(/\s+/).includes("icon") || rel.includes("shortcut icon");
      })
      .map((link) => ({
        href: toAbsoluteUrl(link.getAttribute("href"), baseUrl),
        priority: getIconPriority(link)
      }))
      .filter((item) => item.href)
      .sort((a, b) => b.priority - a.priority)
      .map((item) => item.href);

    const manifestHref = links.find((link) => (link.getAttribute("rel") || "").toLowerCase().includes("manifest"))?.getAttribute("href");
    const candidates = [...directIcons];
    if (manifestHref) {
      candidates.push({ manifest: toAbsoluteUrl(manifestHref, baseUrl) });
    }
    return candidates;
  }

  function getIconPriority(link) {
    const rel = (link.getAttribute("rel") || "").toLowerCase();
    const sizes = (link.getAttribute("sizes") || "").toLowerCase();
    const sizeScore = sizes
      .split(/\s+/)
      .map((size) => Number(size.match(/(\d+)x\d+/)?.[1] || 0))
      .reduce((max, size) => Math.max(max, size), 0);
    const type = (link.getAttribute("type") || "").toLowerCase();
    return (rel.includes("apple-touch-icon") ? 1000 : 0) + Math.min(sizeScore, 512) + (type.includes("svg") ? 80 : 0);
  }
  ...
  async function fetchIconCandidate(candidate) {
    ...
    if (typeof candidate === "object" && candidate.manifest) {
      return fetchManifestIcon(candidate.manifest);
    }
    try {
      const response = await fetch(candidate, { cache: "force-cache", redirect: "follow" });
      if (!response.ok) {
        return "";
      }
  ...
  async function fetchManifestIcon(manifestUrl) {
    try {
      ...
      const candidates = icons
        .map((icon) => ({
          href: toAbsoluteUrl(icon.src, response.url || manifestUrl),
          priority: getManifestIconPriority(icon)
        }))
        .filter((item) => item.href)
        .sort((a, b) => b.priority - a.priority);
      for (const candidate of candidates) {
        const dataUrl = await fetchIconCandidate(candidate.href);
        if (dataUrl) {
          return dataUrl;
        }
      }
  ...
  function getManifestIconPriority(icon) {
    ...
    return Math.min(sizeScore, 512) + (type.includes("svg") ? 80 : 0);
  }
